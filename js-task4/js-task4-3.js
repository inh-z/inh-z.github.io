...
var c//全局变量，用于后边的按钮判断 + 保存为参数传递，确定选中的人
$(".box").click(function(){
...
$("#next").click(function(){
	...
	if(x){//这个判断条件代表选中了一名玩家
		if (peopleAll[c].aLive==1) {//判断存活状态
			if (peopleAll[c].idName==1) {
				peopleAll[c].aLive=2;//规定存货状态中，1是存活、2是被杀手杀死、3是被投死
				peopleAll[c].order=orderNumber;//添加死亡顺序
				orderNumber++;
				die.push(c);//死亡人的id加入数组
				eval("tianShu.dayDetail" + i)[0][1]=1;//激活li
	...
	var r=0;
	var re=0;
	/*for(var u=0;u<killer;u++){//判断条件错误
		if (peopleAll[u].aLive==1) {
			r++;
		}
	}

	for(var u=0;u<normaler;u++){
		if (peopleAll[u].aLive==1) {
			re++;
		}
	}*/
	for(var u=0;u< peopleData.length;u++){
		if(peopleAll[u].idName==0){//判断身份,0是杀手
			if(peopleAll[u].aLive==1){
				r++;
			}
		}
